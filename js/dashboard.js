...
/**
 * Função debounce para otimizar requisições
 */
...
function inicializarBuscaPerfis() {
    ...
    // Busca em tempo real conforme digita (com debounce)
    ...
    console.log("✅ Sistema de busca de perfis inicializado!");
}
...
async function executarBuscaPerfil(username = null) {
    ...
    try {
        ...
        // Renderizar resultados (filtrar nulos)
        searchResults.innerHTML = "";
...
function renderPerfilEncontrado(usuario) {
    ...
    // Tentar obter links_count de diferentes fontes possíveis
    let linksCount = 0;
...
async function buscarUsuarios(searchTerm) {
    try {
        ...
        // Usar a rota de perfil público: GET /user/<username>
        ...
        // Retornar como array para manter compatibilidade com renderPerfilEncontrado
        return data ? [data] : [];
    } catch (error) {
...
function navegarParaPerfil(username) {
    // Usa a short URL que redireciona para profile.html?user=username
    window.location.href = `/${encodeURIComponent(username)}`;
}

function escapeHtml(text) {
    // Validar se text existe e é string
